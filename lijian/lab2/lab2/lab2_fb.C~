...
double forward_backward(const Graph& graph, const matrix<double>& gmmProbs,
	...
	if(i==graph.get_start_state())
		chart(0,i).set_forw_log_prob(0);
	else
		chart(0,i).set_forw_log_prob(g_zeroLogProb);
	}

    // Iterate over all states and if there are arcs between one state to the current state, then calculate the log prob 
    for (int i=1; i<=frmCnt; ++i){
	for (int k=0; k<stateCnt; ++k){  // k is the current state
		vector<double> forwLogProb(stateCnt);
		for (int m=0; m<stateCnt; ++m)
		{
			forwLogProb[m]=g_zeroLogProb;
		}
		for (int j=0; j<stateCnt; ++j){
			int arcCnt=graph.get_arc_count(j); // Get number of outgoing arcs
			int arcId=graph.get_first_arc_id(j); // Get arc ID of first outgoing arc
			for (int arcIdx=0; arcIdx<arcCnt; ++arcIdx){
				Arc arc;
				arcId=graph.get_arc(arcId, arc);
				if(arc.get_dst_state()==k){   // if the dst_state is the current state k
					//cout << format("Time step is: %d\n") %i;
					//cout << format("对于状态 %d ，上一个状态 %d 的跳转概率 %f ， GMM概率 %f , 上一状态的前向概率 %f \n") %k %j %arc.get_log_prob() %gmmProbs((i-1),arc.get_gmm()) %chart(i-1,j).get_forw_log_prob();
					forwLogProb[j]=arc.get_log_prob()+gmmProbs((i-1),arc.get_gmm())+chart(i-1,j).get_forw_log_prob();
				}
			}		
		}
		chart(i,k).set_forw_log_prob(add_log_probs(forwLogProb));
		//cout << format("状态 %d 在当前时刻 %d 的前向概率为 %f\n") %k %i %add_log_probs(forwLogProb);
	}
...
/*
    for (int i=frmCnt; i>=0; --i){
	cout << format("The time step is %d:\n") %i;
	for (int j=0; j<stateCnt; ++j){
		cout << format("%f\n") %chart(i,j).get_back_log_prob();	
	}
    }
*/

    for (int i=frmCnt-1; i>=0; --i){
	for (int j=0; j<stateCnt; ++j){ //current state j
		vector<double> backLogProb(stateCnt);
		for (int m=0; m<stateCnt; ++m)
		{
			backLogProb[m]=g_zeroLogProb;
		}
		int arcCnt=graph.get_arc_count(j);
		int arcId=graph.get_first_arc_id(j);
		for (int arcIdx=0; arcIdx<arcCnt; ++arcIdx){
			Arc arc;
			arcId=graph.get_arc(arcId,arc);
			int dstState=arc.get_dst_state();
			backLogProb[dstState]=arc.get_log_prob()+gmmProbs(i,arc.get_gmm())+chart(i+1, dstState).get_back_log_prob();
		}
		chart(i,j).set_back_log_prob(add_log_probs(backLogProb));		
	}
    }
	...
	for (int stateIdx=0; stateIdx<stateCnt; ++stateIdx){
		...
		for (int arcIdx=0; arcIdx<arcCnt; ++arcIdx){
			...
			arcId=graph.get_arc(arcId,arc);
			int dstState=arc.get_dst_state();
			double arcPosterior=0.0;
			arcPosterior=exp(arc.get_log_prob()+gmmProbs(frmIdx,arc.get_gmm())+chart(frmIdx, stateIdx).get_forw_log_prob()+chart(frmIdx+1, dstState).get_back_log_prob()-uttLogProb);
