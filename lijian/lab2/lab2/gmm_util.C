...
double GmmStats::add_gmm_count(unsigned gmmIdx, double posterior,
    ...
    m_gaussCounts[gaussIdx]=m_gaussCounts[gaussIdx]+1;

    for(int i=0; i<dimCnt; ++i){
    	m_gaussStats1(gaussIdx,i)=posterior*feats[i]+m_gaussStats1(gaussIdx,i);
	m_gaussStats2(gaussIdx,i)=posterior*feats[i]*feats[i]+m_gaussStats2(gaussIdx,i);
	//cout<<format("Dim%d:gaussStats1 is %f gaussStats2 is %f\n") %i %m_gaussStats1(gaussIdx,i) %m_gaussStats2(gaussIdx,i);
    }
...
void GmmStats::reestimate() const
    ...
    //for (int i=0; i<m_gaussCounts.size(); ++i)
    //  cout << format("The GaussianIdx %d is %d\n") %i %m_gaussCounts[i];

    for(int i=0; i<gaussCnt; ++i)
	for(int j=0; j<dimCnt; ++j)
	  {
		m_gmmSet.set_gaussian_mean(i,j,m_gaussStats1(i,j)/m_gaussCounts[i]);
		m_gmmSet.set_gaussian_var(i,j,m_gaussStats2(i,j)/m_gaussCounts[i]-(m_gaussStats1(i,j)/m_gaussCounts[i])*(m_gaussStats1(i,j)/m_gaussCounts[i]));
	  }
